...
const AddSecurityTraining = () => {
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`${process.env.REACT_APP_API_URL}/training/security/${id}`)
        .then((res) => setData(res?.data))
        .catch((err) => console.log(err));
    }
  }, [id]);

  const handleAddSecurity = (data) => {
    console.log(data);
    ...
    if(id) {
        axios
        .put(`${process.env.REACT_APP_API_URL}/training/security/${id}`, formData)
        .then((res) => {
        ...
        axios
          .post(`${process.env.REACT_APP_API_URL}/training/addSecurity`, formData)
          .then((res) => {
  ...
  return (
    <RtCard title={`${id ? 'Edit' : 'Add'} Security Training`}>
      <PartnerPageForm
      form='training'
        data={data}
...
export default AddSecurityTraining;
