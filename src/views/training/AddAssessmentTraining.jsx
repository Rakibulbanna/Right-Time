...
const AddAssessmentTraining = () => {
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`${process.env.REACT_APP_API_URL}/training/assesment/${id}`)
  ...
  const handleAddAssessment = (data) => {
    ...
    if(id) {
        axios
        .put(`${process.env.REACT_APP_API_URL}/training/assessment/${id}`, formData)
        .then((res) => {
        ...
        axios
          .post(`${process.env.REACT_APP_API_URL}/training/addAssessment`, formData)
          .then((res) => {
  ...
  return (
    <RtCard title={`${id ? 'Edit' : 'Add'} Assessment Training`}>
      <PartnerPageForm
        form='training'
        data={data}
...
export default AddAssessmentTraining;
