...
const AddManagementTraining = () => {
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`${process.env.REACT_APP_API_URL}/training/management/${id}`)
        .then((res) => setData(res?.data))
        .catch((err) => console.log(err));
    }
  }, [id]);

  const handleAddManagement = (data) => {
    console.log(data);
    ...
    if (id) {
      axios
        .put(`${process.env.REACT_APP_API_URL}/training/management/${id}`, formData)
        .then((res) => {
      ...
      axios
        .post(`${process.env.REACT_APP_API_URL}/training/addManagement`, formData)
        .then((res) => {
  ...
  return (
    <RtCard title={`${id ? 'Edit' : 'Add'} Management Training`}>
      <PartnerPageForm
        form="training"
        data={data}
        formInputFields={serviceInputFields}
        onSubmit={handleAddManagement}
      />
    </RtCard>
  );
};

export default AddManagementTraining;
